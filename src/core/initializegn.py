...
def run(username, password, school, phone):
    ...
    if constants.is_local():
        ...
        with open("{0}.txt".format(log_path), "w+") as outfile:
            ...
            child.sendline(f'nohup setsid python3 {constants.script_path()}/grade_notifier.py --username={username} --password={password} --school={school} --phone={phone} --prod=true') # execute command
